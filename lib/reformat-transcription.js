...
export default async function generateReformatedTranscription(outputDir, minDuration) {
    const filePath = path.join(outputDir, 'transcription.json')
    const data = fs.readFileSync(filePath, 'utf8')

    // Generowanie treści pliku SRT
    const transcriptionData = JSON.parse(data)
    transcriptionData.words = mergeWordsToPhrases(transcriptionData.words, Number(minDuration))

    fs.writeFileSync(path.join(outputDir, 'transcription-reformatted.json'), JSON.stringify(transcriptionData, null, 2));
}
